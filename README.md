...
```
The selfish behaviour implies that there is no way to guarantee the integrity of the information, since the nodes could express conflicting information. And that's ok because **each node have it's own truth**.
...
Because the reasoning above it is not possible to have ciclic references between coordinates.
...
We call this convergence a `node cluster`. In other words is the set of nodes that are poining to the same `CID`.
